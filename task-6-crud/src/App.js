...
function App() {
  ...
  const [editonCard, setEditoncard] = useState("add");
  ...
  const submit = () => {
    ...
    if(carname, model, price, type, description){
      if (editonCard === "add"){
        let formDetails = {
        id:uuidv4(), carname, model, price, type, description
      }
   
      setCrudDetails([...crudDetails, formDetails])

      }
      else if (editonCard === "edit"){
        let updateFormDetails = crudDetails.filter (data => data.id == id)
        let updateDetails = {
          id, carname, model, price, type, description 
        }
        setCrudDetails([...updateFormDetails, updateDetails]) 
      }
